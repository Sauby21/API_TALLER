...
def postObtenerUnaHistoriaH(id_usu, id_hist):
  try:
    conn = connection()
    ...
    inst =  '''
            select h.id_hist, to_char(h.apertura, 'DD-MM-YYYY') as apertura, h.estado, to_char(h.alta , 'DD-MM-YYYY') as alta
              ...
              where eh.id_est = %(id_usu)s and eh.id_hist = h.id_hist and h.id_hist = %(id_hist)s;
            '''
    
    with conn.cursor() as cursor:
      cursor.execute(inst, {'id_usu': id_usu, 'id_hist': id_hist})
